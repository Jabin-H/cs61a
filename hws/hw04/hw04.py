...
def balanced(m):
    ...
    if is_planet(m):
        ...
        assert is_mobile(m), "must check balance on a mobile or a planet"
        left_end, right_end = end(left(m)), end(right(m))
        torque1, torque2 = length(left(m)) * total_mass(left_end), length(right(m)) * total_mass(right_end)
        if torque1 == torque2 and balanced(left_end) and balanced(right_end):
            return True
        else:
            return False
...
def berry_finder(t):
    ...
    return label(t) == 'berry' or bool(sum([berry_finder(branch) for branch in branches(t)]))
