...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    count, length_t, length_s = 0, len(typed_words), len(source_words)
    if length_t == 0 and length_s == 0:
        return 100.0
    elif length_t == 0 or length_s == 0:
        return 0.0
    for t, s in zip(typed_words, source_words):
        count += (t == s)
    # for i in range(length_t):
    #     s = source_words[i] if i < length_s else ''
    #     if typed_words[i] == s:
    #         count += 1
    return count / length_t * 100
    # END PROBLEM 3
...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        ...
        key = (typed, source)
        if key in cache:
            result, limit_cached = cache[key]
            if limit_cached >= limit:
                return result
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    key_func = lambda w: diff_function(typed_word, w, limit)
    word_diff_min = min(word_list, key=key_func)
    diff_min = key_func(word_diff_min)
    return word_diff_min if diff_min <= limit else typed_word
    # END PROBLEM 5
...
def furry_fixes(typed, source, limit):
    ...
    """
    # BEGIN PROBLEM 6
    length_t, length_s = len(typed), len(source)
    if limit < 0 or (limit == 0 and length_t != length_s):
        return 1
    if length_t == 0 or length_s == 0:
        return abs(length_t - length_s)
    if typed[0] == source[0]:
...
def time_per_word(words, timestamps_per_player):
    ...
    # times = []
    # for stamps in timestamps_per_player:
    #     time = []
    #     for i in range(1, len(stamps)):
    #         time.append(stamps[i] - stamps[i - 1])
    #     times.append(time)
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    return [
        [
            ...
            if min(player_indices, key=lambda player_num: get_time(times, player_num, word_index)) == i
        ]
        for i in player_indices
    ]
    # result = [[] for i in player_indices]
    # for word_index in word_indices:
    #     key_func = lambda palyer_num: get_time(times, palyer_num, word_index)
    #     min_player_index = min(player_indices, key=key_func)
    #     result[min_player_index].append(words[word_index])
    # return result
    # END PROBLEM 10
