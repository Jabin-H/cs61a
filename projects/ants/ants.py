    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if self.can_contain(place.ant):
                container, contained = self, place.ant
            elif place.ant.can_contain(self):
                container, contained = place.ant, self
            container.store_ant(contained)
            place.ant = container
            # END Problem 8b
...
# BEGIN Problem 6
class WallAnt(Ant):
    """WallAnt does nothing but protect other ants like a wall."""
...
# BEGIN Problem 7
class HungryAnt(Ant):
    """HungryAnt chews a Bee each 'chew_cooldown' turns in its place."""
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower is an ant similar to ThrowerAnt, but waterproof."""
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        def slow_action(self, gamestate):
            if self.slow_turn == 0:
                self.action = Bee.action.__get__(self)
        ...
        if target is not None:
            ...
            target.slow_turn = self.slow_turns
        # END Problem EC 1
...
class LaserAnt(ThrowerAnt):
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem EC 4
        return max(self.damage - self.insects_shot * self.shot_factor - distance * self.place_factor, 0)
        # END Problem EC 4
...
class Bee(Insect):
    ...
    def scare(self, length):
        ...
        """
        # BEGIN Problem EC 2
        if not self.scary_before:
            self.scary_turn = length
            self.scary_before = True
        # END Problem EC 2
